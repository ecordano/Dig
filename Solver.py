...
def solve(time, land1, land2, land3, metric):
    ...
    balanced = {'CUC': 5, 'EGG': 5, 'STOM': 5, 'GBEAN': 5, 'BSQ': 5, 'CTOM': 5, 'ZUC': 5, 'WAT': 5, 'BPEP': 5,
    ...
    if metric == "Balanced":
        prob += lpSum([balanced[i] * crop_vars[i] for i in crops]), "The optimal solution is"
    elif metric == "Nutritional Value":
        prob += lpSum([nutrition[i] * crop_vars[i] for i in crops]), "The optimal solution is"
    else:
        prob += lpSum([total_profit[i] * crop_vars[i] for i in crops]), "The optimal solution is"  # The constraints are added to 'prob'
...
def solve_man(time, land1, land2, land3, metric, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15, v16, v17, v18, v19, v20, v21, v22):
    # Creates a list of the produce options
    crops = ['CUC', 'EGG', 'STOM', 'GBEAN', 'BSQ', 'CTOM', 'ZUC', 'WAT', 'BPEP', 'STR', 'POT', 'SCORN', 'CAR', 'BEET',
             'ONS', 'RAD', 'SPEA', 'LET', 'GAR', 'BSPR', 'KALE', 'SPCH']
    ...
    # A dictionary of personal preferences for which plants to grow -> will be user input, could be balanced or nutritional or profit
    # Can we do an if statement here? But how? Need a variable for if user selects bal/nut/or profit/ or own
    manual_preferences = {'CUC': v1, 'EGG': v2, 'STOM': v3, 'GBEAN': v4, 'BSQ': v5, 'CTOM': v6, 'ZUC': v7, 'WAT': v8, 'BPEP': v9,
                          'STR': v10, 'POT': v11, 'SCORN': v12, 'CAR': v13, 'BEET': v14, 'ONS': v15, 'RAD': v16, 'SPEA': v17, 'LET': v18,
                          'GAR': v19, 'BSPR': v20, 'KALE': v21, 'SPCH': v22}
